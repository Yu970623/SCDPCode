function [FrontNo,MaxFNo] = NDSortFdv(PopObj,PopCon,nSort,stage)
...
%--------------------------------------------------------------------------
    ...
    [N,M]  = size(PopObj);
    Infeasible           = any(PopCon>0,2); %不可行解下标3
    CV = FDVOperator_CV(sum(max(0,PopCon(Infeasible,:)),2),stage); 
    PopObj(Infeasible,:) = repmat(max(PopObj,[],1),sum(Infeasible),1) + repmat(CV,1,M);  
        
%         [InfN,~] = size(PopCon(Infeasible,:)); %不可行解个数
%         coverage = sum(PopCon(Infeasible,:)>0,2); %不可行解个数？
%         maxViolation = repmat(max(abs(PopCon)),InfN,1); %最大约束违反值
%         distance = sum(max(0, PopCon(Infeasible,:)./(maxViolation+1e-308)),2);
%         PopObj(Infeasible,:) = repmat(max(PopObj,[],1),sum(Infeasible),1) + repmat(coverage+distance,1,M);
    if M < 3 || N < 500

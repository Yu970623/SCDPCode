...
% This function is written by Kangjia Qiao
    ...
    methods
        function main(Algorithm,Problem)
            ...
            k = 1;
            I = [];
            S = IGD( Population{1},Problem.optimum);
            I = [I,S];
            %% Optimization
            while Algorithm.NotTerminated(Population{1})
                ...
                if Problem.FE/Problem.N == gen(k) || Problem.FE >= Problem.maxFE-3*Problem.N
                    pop_gen = gen(k); % 获取类名
                    pop = Population{1}.objs;
                    fileName1 = sprintf('D:\\experiment\\PlatEMO-EPDCMO\\Gen\\URCMO_LIR9_%d.mat', pop_gen);
